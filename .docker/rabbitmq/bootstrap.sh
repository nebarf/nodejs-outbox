...
rabbitmqadmin declare queue name=order.events durable=true -u $RABBITMQ_ADMIN_USER -p $RABBITMQ_ADMIN_PASSWORD ; \
rabbitmqadmin declare binding source="order.events" destination_type="queue" destination="order.events" -u $RABBITMQ_ADMIN_USER -p $RABBITMQ_ADMIN_PASSWORD ; ) &
